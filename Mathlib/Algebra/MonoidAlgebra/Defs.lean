...
def curryRingEquiv : R[M × N] ≃+* R[N][M] :=
  .symm {
    toAddEquiv := coeffAddEquiv.trans <| .trans (Finsupp.mapRange.addEquiv coeffAddEquiv) <| .trans
      curryAddEquiv.symm coeffAddEquiv.symm
    map_mul' x y := by
      classical
      ext ⟨m, n⟩
      simp only [AddEquiv.toEquiv_eq_coe, Equiv.toFun_as_coe, EquivLike.coe_coe,
        AddEquiv.trans_apply, coeffAddEquiv_apply, mapRange.addEquiv_apply,
        curryAddEquiv_symm_apply, coeffAddEquiv_symm_apply, Finsupp.uncurry_apply_pair,
        mapRange_apply, coeff_mul, Finsupp.sum_comm _ y.coeff, coeff_finsuppSum, apply_ite,
        coeff_zero, sum_apply, coe_zero, ite_apply, Pi.zero_apply, sum_uncurry_index,
        sum_zero_index, implies_true, sum_mapRange_index, Prod.mk_mul_mk, Prod.mk.injEq, ite_and]
      congr! with m₁ z₁ m₂ z₂
      split <;> simp
  }
